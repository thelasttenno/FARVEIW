...
function SolarSystem(props, { dt = 0.1 }) {
  ...
  const xInitialArray = data.planets.map((planet) =>
    planet.x.map((xyz) => xyz * 1)
  );
  const vInitialArray = data.planets.map((planet) =>
    planet.v.map((xyz) => xyz * 1)
  );
